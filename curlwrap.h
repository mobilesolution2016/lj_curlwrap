...
public:
	...
	std::string			strDumpUpload, strDumpResponse;
...
static CURLcode curlExecOne(CURLUserData* user)
{
	...
	if (user->strDumpResponse.length())
	{
		FILE* fp = fopen(user->strDumpResponse.c_str(), "wb");
		if (fp)
		{
			fwrite(user->strResponse.c_str(), 1, user->strResponse.length(), fp);
			fclose(fp);
		}
	}
